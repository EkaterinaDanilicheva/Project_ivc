<?php
/* Этот скрипт считает сколько и на кого назвонил Баранов и сколько и кто звонил на Баранова
$num_arr это нумерные емкости операторов*/
...
$dbh = mysql_connect($eltex_host, $eltex_user, $eltex_password) or die("Не могу соединиться с MySQL.");
mysql_select_db($eltex_database) or die("Не могу подключиться к базе.");
	...
	if( $mykey === "baranov_on" ) { //форминуем начало запроса (SIP и номера Баранова)
		...
		foreach ( $array as $caption => $sip ) { // массив по заголовкам
			...
			while($row = mysql_fetch_assoc($res)) {
					...
					$eltex_min_arr[$caption] = $row['min'];
					$eltex_sec_arr[$caption] = $row['sec'];
		...
		$baranov_query_str = str_replace(" OR)", " )", $baranov_query_str . ")");
			...
			while($row = mysql_fetch_assoc($res)) {
					...
					$eltex_min_arr['Баранов на всё, что не попало под предыдущие'] = $row['min'];
					$eltex_sec_arr['Баранов на всё, что не попало под предыдущие'] = $row['sec'];
		...
		$baranov_query_str = str_replace(" OR)", " )", $baranov_query_str . ")") . " GROUP BY source_type";
			...
			while($row = mysql_fetch_assoc($res)) {
					...
					$eltex_min_arr['на Баранова От всех остальных по источнику '. $row['callers_name']] = $row['min'];
					$eltex_sec_arr['на Баранова От всех остальных по источнику '. $row['callers_name']] = $row['sec'];
...
foreach ($eltex_min_arr as $eltex_oper => $eltex_value) {
	...
	$table = $table . "<tr>
		<td>$eltex_oper</td>
		<td>$eltex_value мин.</td>
		<td>". $eltex_sec_arr[$eltex_oper] ." сек.</td>
	</tr>";
}
$table = $table . "</table>";
///////////////////////////////////////
